...
async function handler(m, { conn, args }) {
    ...
    if (m.sender in confirm) throw 'Youre still gambling, wait until its over!!'
    try {
...
YOU *${status}*, YOU ${status == 'WIN' ? `Get *+${count * 2}*` : status == 'Lost' ? `lost  *-${count * 1}*` : `Get *+${Math.floor(count / 1.5)}*`} 💵Money
    `.trim())
            clearTimeout(timeout)
            delete confirm[m.sender]
            return !0
